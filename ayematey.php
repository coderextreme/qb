...
class IRCBot {
	...
	public $count30 = 0;
	public $count480 = 0;
	public $specialword = "hi";

	function pickNewWord() {
		$specialwords = array();
			$file = fopen("shivers.txt", "r+");
			while (($word = fgets($file, 8192)) != FALSE) {
				array_push($specialwords, trim($word));
			}
			fclose($file);
		$this->specialword = $specialwords[rand(0, count($specialwords)-1)];
	}
	...
	function peewee($to, $word, $nick) {
		$this->DelaySend($to, "Shiver me timbers!");
	}

	function title($to, $url, $nick) {
		echo $to."\n";
		$url = trim($url);
		...
		if (ini_get('allow_url_fopen') == 1) {
		    echo 'fopen is allowed on this host.\n';
		} else {
		    echo 'fopen is not allowed on this host.\n';
		}
		...
		foreach ($items as $item) {
			$this->DelaySend($to, "Title from ".$nick."'s link: ".$item->nodeValue);
		}
	}
	function loadTaoTe() {
		$doc = new DOMDocument();
	...
	function checkForMessages() {
		if ($this->online && !feof($this->fp)) {
			...
			if ($array[0] == "PING") {
				...
				$this->pinged = true;
				if (!$this->quiet && ($this->count > 8 || $this->count30 > 15 || $this->count480 > 480)) {
					$this->RandomQuote($this->tag);
					$this->count = 0;
					$this->count30 = 0;
					if ($this->count480 > 480) {
						$this->pickNewWord();
						$this->count480 = 0;
					}
				} else {
	...
	function ProcessMessage($array) {
		...
		$this->count = 0;
		if (preg_match('/https?:\/\/[^ \t]*/', $capturedMessage, $matches)) {
			$this->title($to, $matches[0], $nick);
		}
		if (preg_match('/\b'.$this->specialword.'\b/', $capturedMessage)) {
			if ($to[0] == "#") {
				$this->peewee($to, $this->specialword, $nick);
			}
		}
